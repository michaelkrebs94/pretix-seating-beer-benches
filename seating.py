...
class Config:
    seats_per_table: int = 10 # Number of seats per table
    num_tables_x: int = 9 # Number of tables next to each other
    num_tables_y: int = 11 # Number of tables in a row
    gap_tables_x: int = [20, 40] # Gaps between tables in x direction, alternating through the list
    gap_tables_y: int = [5,5,5,40] # Gaps between tables in y direction, alternating through the list
...
def create_table(table: int) -> dict:
...
def create_seating() -> dict:
    seating = [create_table(i) for i in range(1, config.num_tables_x * config.num_tables_y + 1)]
    return {
        "name": "Seating",
        "categories": [],
        "zones": [
        ...
        "size": {
            "width": max([table["position"]["x"] + seat["position"]["x"] for table in seating for seat in table["seats"]]) + config.seat_radius + config.margin * 2,
            "height": max([table["position"]["y"] + seat["position"]["y"] for table in seating for seat in table["seats"]]) + config.seat_radius + config.margin * 2
        }
    }
